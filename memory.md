...
Decided 2026-09-23: policies and content rules share `log < alert < warn < steer < redact < require_approval < block`, with block levels `tool_call < turn < revoke_key < stop < quarantine` (the #394 table). The policy mode stays the ceiling. Audit policies keep `action=block` and the ceiling records them as `log`, so switching one to enforce still blocks; audit performs only the `log`/`alert` steps a rule reached. `ContentRule.enforcement` was replaced by `action` (GraphQL still accepts and returns `enforcement` as a legacy alias). The column itself stays for one release as a mirror written on every save, and the new columns old pods insert into have database defaults, because the backend migrates at startup while the previous release still serves (review of #411); #416 drops the column. Severity escalation reads the evaluation's risk score through the incident severity mapping, because policies carry no severity of their own. That makes it advisory for policies: the score reads flags the caller declares. A rule Zentinelle could not evaluate (an evaluator that raised, unreadable selectors) refuses the call under enforce whatever its action. Fallback never weakens what a rule meant to stop: a block falls back upward and then to refusing the call; steer falls back to warn. `allowed` stays the capability-agnostic floor: nothing a caller declares changes it. Redact denies on `/evaluate`, because no policy evaluator returns redacted content to pass on (review of #411: letting `supports_redact` allow it turned any redact policy into an allow for a caller that claimed it). Existing rules are proven unchanged by a golden file recorded on main before the change and replayed through the real migration. Delivery of steer and stop is Astrolift's (#394, astrolift-app#1903). Follow-ups: #401, #404, #407.
