...
export default function ControlEvidencePage() {
  ...
  return <div className="space-y-6 p-6">
    ...
    <Card><CardHeader><CardTitle>Control coverage states</CardTitle></CardHeader><CardContent>{data ? <div className="flex flex-wrap gap-3">{Object.entries(data.coverage).map(([status, count]) => <div key={status} className="flex items-center gap-2 text-sm"><Badge variant={status === "unknown" || status === "unsupported" ? "secondary" : "default"}>{status}</Badge><span>{count} evidence records</span></div>)}</div> : <p className="text-muted-foreground text-sm">No control evidence available.</p>}</CardContent></Card>
  </div>;
}
