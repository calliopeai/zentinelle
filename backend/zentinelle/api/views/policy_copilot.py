...
from zentinelle.models import (AuditLog, ControlEvidence, LLMProviderKey,
                               Policy, PolicyChangeSet, TenantConfig)
from zentinelle.schema.auth_helpers import get_request_tenant_id

_SENSITIVE_CONFIG_KEY = re.compile(
    r'(?:secret|token|password|credential|api[_-]?key)', re.I)
...
def _redact_config(value, depth=0):
    ...
    if isinstance(value, dict):
        return {
            str(key): (
                '[redacted]' if _SENSITIVE_CONFIG_KEY.search(
                    str(key)) else _redact_config(
                    item,
                    depth +
                    1)) for key,
            item in value.items()}
...
def _validate_scope(payload, tenant_id):
    ...
    if requested_tenant is not None and str(
    ...
    if client_id is not None and (
        not isinstance(
            client_id,
            str) or not client_id.strip() or len(client_id) > 255):
...
class PolicyCopilotDraftView(APIView):
    ...
    def post(self, request):
        ...
        if not values.get('policy_copilot_enabled', False):
            return JsonResponse(
                {'error': 'Policy copilot is disabled'}, status=403)
        if not _has_assistant_key(tenant_id):
            return JsonResponse(
                {'error': 'Configure an active provider key before using the copilot'}, status=503)
        ...
        if natural_language:
            ...
            if not prompt_check.allowed:
                ...
                return JsonResponse(
                    {'error': 'policy_copilot_prompt_denied'}, status=422)
        if natural_language and (
            not policy_type or not isinstance(
                draft_config, dict)):
        ...
        if errors:
            return JsonResponse(
                {'error': 'Invalid policy draft', 'fields': errors}, status=400)
        ...
        draft = {
            ...
            'scope_type': payload.get(
                'scope_type',
                Policy.ScopeType.ORGANIZATION),
            'enforcement': payload.get(
                'enforcement',
    ...
    def _infer_draft(prompt):
        ...
        if 'model' in text and any(
            word in text for word in (
                ...
                'block')):
            models = [
                token.strip('.,') for token in text.split() if token.startswith(
                    ('gpt-', 'claude-', 'gemini-'))]
            return 'model_restriction', {
                'allowed_models': models} if models else {
                'allowed_models': [], 'review_required': True}
        if 'tool' in text and any(
            word in text for word in (
                'block', 'deny', 'restrict')):
            tools = [token.strip('.,')
                     for token in text.split() if '_' in token]
            return 'tool_permission', {
                'denied_tools': tools, 'review_required': True}
...
class PolicyCopilotExplainView(APIView):
    ...
    def post(self, request):
        ...
        if not (
            config and (
                config.settings or {}).get(
                'policy_copilot_enabled',
                False)):
            return JsonResponse(
                {'error': 'Policy copilot is disabled'}, status=403)
        payload = request.data if isinstance(request.data, dict) else {}
        ...
        endpoint = AgentEndpoint.objects.filter(
            tenant_id=tenant_id, agent_id=agent_id).first()
        if endpoint is None:
        ...
        decision = PolicyEngine().evaluate(
            endpoint, action, context=payload.get(
                'context', {}) or {}, dry_run=True)
        control_ids = [policy.policy_type for policy in policies]
        evidence = ControlEvidence.objects.filter(
            tenant_id=tenant_id, control_id__in=control_ids).order_by('-captured_at')
        evidence_by_control = {}
...
class PolicyCopilotDiffView(APIView):
    ...
    def post(self, request):
        ...
        draft = payload.get('draft') if isinstance(
            payload.get('draft'), dict) else payload
        current = Policy.objects.filter(
            tenant_id=tenant_id,
            id=policy_id).first() if policy_id else None
        ...
        after = {
            'policy_type': draft.get('policy_type') or (
                current.policy_type if current else None),
            'scope_type': draft.get('scope_type') or (
                current.scope_type if current else Policy.ScopeType.ORGANIZATION),
            'enforcement': draft.get('enforcement') or (
                current.enforcement if current else Policy.Enforcement.ENFORCE),
            'config': draft.get(
                'config',
                {})}
        valid_types = {choice[0] for choice in Policy.PolicyType.choices}
        valid_scopes = {choice[0] for choice in Policy.ScopeType.choices}
        valid_enforcement = {choice[0]
                             for choice in Policy.Enforcement.choices}
        ...
        if client_id:
            impacted_query = impacted_query.filter(
        ...
        try:
            ...
            simulation = simulate_policy(
                tenant_id, after, lookback_days=7, max_events=1000)
        ...
        AuditLog.objects.create(
            ...
            metadata={
                ...
                'changed': changed,
                'simulation': {
                    key: simulation.get(key) for key in (
                        'total_events',
                        'would_block',
                        'would_warn',
                        'inconclusive') if key in simulation}})
        safe_before = {
            **before,
            ...
            **after,
            'config': _redact_config(
                after.get(
                    'config',
                    {}))}
        return JsonResponse({'policy_id': policy_id or None,
                             'before': safe_before,
                             'after': safe_after,
                             'changed_fields': changed,
                             'impacted_agent_count': impacted,
                             ...
                             'mutated': False,
                             'next_step': 'Submit through staged policy workflow'})
...
class PolicyCopilotStageView(APIView):
    ...
    def post(self, request):
        ...
        if (
            not isinstance(
                draft,
                dict) or draft.get('policy_type') not in valid_types or draft.get(
                'scope_type',
                Policy.ScopeType.ORGANIZATION) not in valid_scopes or draft.get(
                'enforcement',
                Policy.Enforcement.ENFORCE) not in valid_enforcement or not isinstance(
                    draft.get(
                        'config',
                        {}),
                dict)):
            return JsonResponse(
                {'error': 'A valid structured draft is required'}, status=400)
        actor = str(
            getattr(
                request.user,
                'pk',
        ...
        return JsonResponse(
            {
                ...
                'mutated_live_policy': False,
                'next_step': 'Validate, stage, approve, and promote through policy workflow'},
            status=201)
