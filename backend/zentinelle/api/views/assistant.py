...
class AssistantChatView(APIView):
    ...
    def _build_system_prompt(self, request, page_context):
        ...
        system = (
            ...
            "You help with policy violations, policy configuration, "
            "compliance frameworks (SOC2, GDPR, HIPAA, EU AI Act, NIST), "
            "risk and incident analysis, content scanning rules, and "
            "the policy inheritance model (Org > Team > Deployment > "
            "Endpoint > User).\n\n"
            f"The user is currently viewing: "
            f"{page_context or 'the dashboard'}\n\n"
            "FORMATTING RULES:\n"
            "- Be terse. The chat panel is narrow (~400px wide).\n"
            "- DO NOT use emojis or unicode icons. Ever.\n"
            "- Use markdown: **bold** for emphasis, `code` for "
            "identifiers, bullet lists for enumeration.\n"
            "- Reference specific policies, agents, or events by name "
            "when relevant.\n"
            "- Skip greetings and pleasantries — answer the question "
            "directly."
        )
