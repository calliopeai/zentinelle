...
def _valid_taxonomy_extension(value):
    if not isinstance(value, str) or not 3 <= len(
            value) <= 128 or value.count(':') != 1:
        return False
...
def _validate_updates(updates):
    ...
    if 'model_visibility' in updates and updates['model_visibility'] not in {
            'enabled_only', 'all_registered', 'approved_only'}:
        return 'model_visibility must be enabled_only, all_registered, or approved_only'
    if 'discovery_refresh_seconds' in updates and (
        not isinstance(
            updates['discovery_refresh_seconds'],
            int) or isinstance(
            updates['discovery_refresh_seconds'],
            bool) or not 300 <= updates['discovery_refresh_seconds'] <= 86400):
        return 'discovery_refresh_seconds must be an integer from 300 to 86400'
    for key, low, high in (('default_rate_limit_per_minute',
                           1, 100000), ('default_budget_cents', 0, 100000000)):
        if key in updates and (
            not isinstance(
                updates[key],
                int) or isinstance(
                updates[key],
                bool) or not low <= updates[key] <= high):
            return f'{key} must be an integer from {low} to {high}'
    if 'content_capture_mode' in updates and updates['content_capture_mode'] not in {
            'metadata', 'redacted', 'full'}:
        return 'content_capture_mode must be metadata, redacted, or full'
    if 'assistant_allowed_topics' in updates and (
        not isinstance(
            updates['assistant_allowed_topics'],
            list) or len(
            updates['assistant_allowed_topics']) > 100 or not all(
                isinstance(
                    item,
                    str) and 0 < len(item) <= 128 for item in updates['assistant_allowed_topics'])):
        return 'assistant_allowed_topics must contain at most 100 bounded strings'
    for key in ('assistant_model', 'assistant_provider'):
        if key in updates and (
            not isinstance(
                updates[key], str) or len(
                updates[key]) > 128):
            return f'{key} must be a bounded string'
    if 'taxonomy_extensions' in updates and (
        not isinstance(
            updates['taxonomy_extensions'], list) or len(
            updates['taxonomy_extensions']) > 200 or not all(
                _valid_taxonomy_extension(item) for item in updates['taxonomy_extensions'])):
...
class RuntimeSettingsView(APIView):
    ...
    def get(self, request):
        ...
        values = {key: stored.get(key, factory())
                  for key, factory in SETTING_DEFAULTS.items()}
        latest = RuntimeSettingsRevision.objects.filter(
            tenant_id=tenant_id).order_by('-revision').first()
        revisions = RuntimeSettingsRevision.objects.filter(tenant_id=tenant_id)[
            :20]
        all_revisions = RuntimeSettingsRevision.objects.filter(
            tenant_id=tenant_id).order_by('-revision')
        effective = {}
        for key in SETTING_DEFAULTS:
            changed = next(
                (row for row in all_revisions if key in (
                    row.settings or {})), None)
            effective[key] = {
                'value': values[key],
                ...
                'changed_by': {
                    'id': changed.actor_id,
                    'name': changed.actor_name} if changed else None,
            }
        pending = RuntimeSettingsChange.objects.filter(
            tenant_id=tenant_id, status__in=[
                RuntimeSettingsChange.Status.STAGED, RuntimeSettingsChange.Status.APPROVED], )[
            :20]
        return JsonResponse({'settings': values,
                             'allowedKeys': list(SETTING_DEFAULTS),
                             ...
                             'revisions': [{'revision': row.revision,
    ...
    def patch(self, request):
        ...
        if unknown:
            return JsonResponse(
                {'error': 'Unknown or unsafe settings', 'keys': unknown}, status=400)
        if 'model_visibility' in updates and updates['model_visibility'] not in {
                'enabled_only', 'all_registered', 'approved_only'}:
            return JsonResponse(
                {'error': 'model_visibility must be enabled_only, all_registered, or approved_only'}, status=400)
        if 'discovery_refresh_seconds' in updates and (
            not isinstance(
                updates['discovery_refresh_seconds'],
                int) or isinstance(
                ...
                bool) or not 300 <= updates['discovery_refresh_seconds'] <= 86400):
            return JsonResponse(
                {'error': 'discovery_refresh_seconds must be an integer from 300 to 86400'}, status=400)
        for key, low, high in (
                ('default_rate_limit_per_minute', 1, 100000), ('default_budget_cents', 0, 100000000)):
            if key in updates and (
                not isinstance(
                    updates[key],
                    int) or isinstance(
                    ...
                    bool) or not low <= updates[key] <= high):
                return JsonResponse(
                    {'error': f'{key} must be an integer from {low} to {high}'}, status=400)
        if 'content_capture_mode' in updates and updates['content_capture_mode'] not in {
                'metadata', 'redacted', 'full'}:
            return JsonResponse(
                {'error': 'content_capture_mode must be metadata, redacted, or full'}, status=400)
        if 'assistant_allowed_topics' in updates and not isinstance(
                updates['assistant_allowed_topics'], list):
            return JsonResponse(
                {'error': 'assistant_allowed_topics must be a list'}, status=400)
        if 'assistant_allowed_topics' in updates and (len(updates['assistant_allowed_topics']) > 100 or not all(
                isinstance(item, str) and 0 < len(item) <= 128 for item in updates['assistant_allowed_topics'])):
            return JsonResponse(
                {'error': 'assistant_allowed_topics must contain at most 100 bounded strings'}, status=400)
        for key in ('assistant_model', 'assistant_provider'):
            if key in updates and (
                not isinstance(
                    updates[key], str) or len(
                    updates[key]) > 128):
                return JsonResponse(
                    {'error': f'{key} must be a bounded string'}, status=400)
        if 'taxonomy_extensions' in updates:
            if (
                not isinstance(
                    updates['taxonomy_extensions'],
                    list) or len(
                    updates['taxonomy_extensions']) > 200 or not all(
                    _valid_taxonomy_extension(item) for item in updates['taxonomy_extensions'])):
                return JsonResponse(
                    {'error': 'taxonomy_extensions must be a list of strings'}, status=400)
        if 'policy_copilot_enabled' in updates and not isinstance(
                updates['policy_copilot_enabled'], bool):
            return JsonResponse(
                {'error': 'policy_copilot_enabled must be boolean'}, status=400)
        if 'control_approval_required' in updates and not isinstance(
                updates['control_approval_required'], bool):
            return JsonResponse(
                {'error': 'control_approval_required must be boolean'}, status=400)
        tenant_id = _tenant_id(request)
        ...
        with transaction.atomic():
            ...
            if expected is not None and expected != current_revision:
                return JsonResponse(
                    {'error': 'stale settings revision', 'revision': current_revision}, status=409)
            ...
            RuntimeSettingsRevision.objects.create(
                tenant_id=tenant_id, revision=revision, settings=config.settings, actor_id=str(
                    getattr(
                        actor, 'pk', '') or ''), actor_name=str(
                    getattr(
                        actor, 'username', '') or ''))
            try:
                ...
                AuditLog.log(
                    tenant_id=tenant_id,
                    action='runtime_settings.changed',
                    resource_type='tenant_runtime_settings',
                ...
                # Settings remain durable even if audit projection is
                # unavailable.
                pass
        return JsonResponse({'settings': {key: config.settings.get(key, factory(
        )) for key, factory in SETTING_DEFAULTS.items()}, 'revision': revision})
...
class RuntimeSettingsRollbackView(RuntimeSettingsView):
    def post(self, request):
        ...
        if not isinstance(
                ...
                int) or isinstance(
                expected_revision,
                bool) or expected_revision < 0:
            return JsonResponse(
                {'error': 'expected_revision is required for rollback'}, status=400)
        tenant_id = _tenant_id(request)
        source = RuntimeSettingsRevision.objects.filter(
            tenant_id=tenant_id, revision=target).first()
        if source is None:
            return JsonResponse({'error': 'revision not found'}, status=404)
        body = json.dumps({'settings': source.settings,
                           'expected_revision': expected_revision}).encode()
...
class RuntimeSettingsChangesView(APIView):
    ...
    def get(self, request):
        ...
        return JsonResponse(
            {'results': [_serialize_change(row) for row in rows]})
    ...
    def post(self, request):
        ...
        actor = str(
            getattr(
                ...
                'username',
                '') or 'operator')
        row = RuntimeSettingsChange.objects.create(
            tenant_id=tenant_id,
            settings=updates,
            base_revision=latest.revision if latest else 0,
            created_by=actor)
...
class RuntimeSettingsChangeTransitionView(APIView):
    ...
    def post(self, request, change_id):
        tenant_id = _tenant_id(request)
        try:
            row = RuntimeSettingsChange.objects.get(
                id=change_id, tenant_id=tenant_id)
        except RuntimeSettingsChange.DoesNotExist:
            return JsonResponse(
                {'error': 'Runtime settings change not found'}, status=404)
        try:
            payload = json.loads(request.body or '{}')
        ...
        try:
            if next_status == RuntimeSettingsChange.Status.APPLIED:
                from zentinelle.services.runtime_settings_changes import \
                    apply_change
