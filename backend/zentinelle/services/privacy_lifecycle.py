...
def erase_tenant(tenant_id, *, actor='privacy-operator'):
    ...
    with tenant_retention_lock(tenant_id):
        ...
        analytics_ok = erase_tenant_analytics(tenant_id)
        if _get_clickhouse_url() and not analytics_ok:
            raise RuntimeError('ClickHouse privacy erasure could not be confirmed')
        ...
        counts = {}
        for model in models:
            deleted, _ = model.objects.filter(tenant_id=tenant_id).delete()
            counts[model.__name__] = deleted
        # Remove only verified local archive files belonging to this tenant.
        archived = 0
        for outcome in RetentionOutcome.objects.filter(tenant_id=tenant_id, status=RetentionOutcome.Status.ARCHIVED):
        ...
        try:
            ...
            pass
        return {'tenant_id': tenant_id, 'records': counts, 'archives_deleted': archived,
