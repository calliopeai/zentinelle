...
class AgentTaxonomyTests(SimpleTestCase):
    def test_browser_contract_fixture_matches_normalization(self):
        fixture = json.loads(
            (Path(__file__).parents[3] /
             'tests/contracts/agent-taxonomy-browser.json').read_text())
    ...
    def test_composes_multiple_scopes_and_preserves_unsupported_tags(self):
        ...
        self.assertEqual(
            result['supported'], [
                'function:customer_service', 'mode:workflow', 'service:legal'])
        self.assertEqual(
            result['unsupported'], [
                'project:acme', 'workspace:claims'])
