...
class RuntimeCoverageTests(TestCase):
    ...
    def test_inventory_distinguishes_observed_and_unknown_workloads(self):
        ...
        by_agent = {row['agent_id']: row['status']
                    for row in result['workloads']}
        self.assertEqual(
            by_agent, {
                'observed': 'observed', 'unknown': 'unknown'})
        observed_row = next(
            row for row in result['workloads'] if row['agent_id'] == 'observed')
        self.assertEqual(
