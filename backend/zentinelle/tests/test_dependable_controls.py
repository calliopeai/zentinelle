...
class DependableControlsTests(TestCase):
    def setUp(self):
        ...
        self.user = get_user_model().objects.create_user(
            username='review-viewer', password='test-only-pass')
    ...
    def test_gateway_can_derive_identity_from_key_but_cannot_impersonate(self):
        ...
        with patch('zentinelle.api.views.evaluate.EvaluateView._log_evaluation'), patch('zentinelle.api.views.evaluate.EvaluateView._log_interaction'):
            ...
            response = self.client.post(
                API + 'evaluate', {'agent_id': 'someone-else', 'action': 'llm:invoke'}, format='json')
    ...
    def test_alias_actions_are_canonicalized_before_policy_evaluation(
            self, evaluate, _log_interaction, _log_evaluation):
        ...
        self.assertEqual(
            evaluate.call_args.kwargs['context']['trace_id'],
        ...
        for agent in (False, True):
            ...
            for path, payload in [
                ('settings/llm-providers', {
                    'provider': 'openai', 'apiKey': 'test-only'}), ('incidents/', {
                        'title': 'Unauthorized'})]:
    ...
    def test_session_mutations_require_csrf_and_valid_token_works(self):
        ...
        self.assertEqual(
            client.patch(
                ...
                payload,
                format='json',
                HTTP_X_CSRFTOKEN=token).status_code,
            200)

    def test_agent_containment_requires_exact_human_approval_when_enabled(
            self):
        assign_role(self.user, ROLE_ADMIN)
        TenantConfig.objects.create(
            tenant_id=TENANT, settings={
                'control_approval_required': True})
        self.client.force_login(self.user)
        url = API + f'agents/{self.endpoint.agent_id}/control'
        action_context = {
            'agent_id': self.endpoint.agent_id,
            'action': 'contain_tools',
            'denied_tools': ['shell']}
        denied = self.client.post(
            url, {'action': 'contain_tools', 'denied_tools': ['shell']}, format='json')
        ...
        token = issue_approval(
            ...
            subject=str(
                self.user.pk),
            action='agent_control:contain_tools',
            context=action_context,
            granted_by=self.user.pk)
        approved = self.client.post(url, {'action': 'contain_tools', 'denied_tools': [
                                    'shell'], 'approval_token': token}, format='json')
        self.assertEqual(approved.status_code, 200)
        self.assertEqual(
            approved.json()['metadata']['containment']['denied_tools'],
            ['shell'])
        replay = self.client.post(url, {'action': 'contain_tools', 'denied_tools': [
                                  'shell'], 'approval_token': token}, format='json')
    ...
    def test_agent_control_detail_exposes_containment_and_decision_traces(
            self):
        from zentinelle.models import AuditLog
        assign_role(self.user, ROLE_ADMIN)
        self.endpoint.metadata = {
            'taxonomy': {
                'supported': ['authority:read_only']},
            'containment': {
                'denied_tools': ['shell']},
            'identities': ['svc:coder'],
            'model_routes': ['openai/gpt-4o'],
            'data_access': ['repo:read'],
            'budget': {
                'monthly_usd': 25}}
        ...
        response = self.client.get(
            API + f'agents/{self.endpoint.agent_id}/control')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.json()['agent']['containment']['denied_tools'],
            ['shell'])
        self.assertEqual(
            response.json()['agent']['model_routes'],
            ['openai/gpt-4o'])
        self.assertEqual(response.json()['agent']['budget']['monthly_usd'], 25)
        self.assertEqual(
            response.json()['decision_traces'][0]['trace_id'],
            'trace-control')
    ...
    def test_evaluation_contract_uses_canonical_boundary_resource_types(self):
        ...
        self.assertEqual(
            EvaluateView._resource_type_for_action('mcp.tool_call'),
            'tool')
        self.assertEqual(
            EvaluateView._resource_type_for_action('rag.retrieve'),
            'retrieval')
        self.assertEqual(EvaluateView._resource_type_for_action(
            'workflow.transition'), 'workflow')
        self.assertEqual(
            EvaluateView._resource_type_for_action('network.egress'),
            'egress')
    ...
    def test_oidc_subject_does_not_link_by_email_and_demotion_revokes_admin(
            self):
    ...
    def test_independent_constraints_compose_and_specific_override_wins(self):
        ...
        broad.save()
        self.assertIn(
            broad, PolicyEngine().get_effective_policies(
                self.endpoint, use_cache=False))
    ...
    def test_team_policy_is_resolved_from_workload(self):
        ...
        self.policy(
            config={
                'denied_actions': ['tool:delete']},
            scope_type='sub_organization',
            scope_sub_organization_id_ext='team-a')
        self.assertFalse(
            PolicyEngine().evaluate(
                self.endpoint,
                'tool:delete').allowed)
    ...
    def test_full_input_and_output_are_inspected(self):
        ...
        self.assertFalse(
            PolicyEngine().evaluate(
                self.endpoint, 'llm:response', context={
                    'output_text': 'Contact jane@example.com'}).allowed)
        self.assertFalse(
            PolicyEngine().evaluate(
                self.endpoint,
                'llm:response',
        ...
        normalized = normalize_context(
            {'request_body': json.dumps({'messages': [{'content': text}]})})
    ...
    def test_retention_preserves_holds_and_checkpoint_verification(self):
        ...
        self.assertTrue(
            verify_chain(
                TENANT,
                expected_checkpoint=pinned)['valid'])

    def test_budget_uses_real_scope_and_consumes_capacity_before_execution(
            self):
        ...
        policy = Policy.objects.create(
            ...
            config={
                'monthly_limit_usd': .03})
        context = {
            'model': 'gpt-4o',
            'input_text': 'hello',
            'max_output_tokens': 1,
            'request_id': 'one',
            'current_month_spend_usd': -10000}
        engine = PolicyEngine()
        self.assertTrue(
            engine.evaluate(
                self.endpoint,
        ...
        policy.scope_endpoint = AgentEndpoint.objects.create(
            tenant_id=TENANT, agent_id='other', name='Other')
        policy.save()
        self.assertTrue(
            engine.evaluate(
                self.endpoint,
    ...
    def test_capture_does_not_persist_prompt_content_by_default(self):
        ...
        entry = InteractionLog.objects.create(
            tenant_id=TENANT,
            interaction_type='chat',
            user_identifier='user',
            occurred_at=timezone.now(),
            input_content='jane@example.com',
    ...
    def test_graphql_viewer_cannot_mutate_but_operator_can_create(self):
        ...
        denied = self.client.post(
            '/gql/zentinelle/', {'query': query}, format='json').json()
        self.assertEqual(
...
class ConcurrentAdmissionTests(TransactionTestCase):
    def test_parallel_requests_cannot_spend_the_same_remaining_budget(self):
        ...
        endpoint = AgentEndpoint.objects.create(
            tenant_id=TENANT, agent_id='parallel-budget', name='Parallel')
        Policy.objects.create(
            tenant_id=TENANT,
            name='Cap',
            policy_type='budget_limit',
            config={
                'monthly_budget_usd': .03})
        barrier = Barrier(2)
    ...
    def test_only_one_parallel_request_can_use_an_approval(self):
        ...
        from django.db import connections
        endpoint = AgentEndpoint.objects.create(
            tenant_id=TENANT, agent_id='parallel-approval', name='Parallel')
        policy = Policy.objects.create(
            tenant_id=TENANT,
            name='Approval',
            policy_type='agent_capability',
            config={
                'require_approval': ['tool:send']})
        token = issue_approval(
            tenant_id=TENANT,
            kind='policy',
            subject='user',
        ...
        def invoke(_):
            try:
                barrier.wait(timeout=10)
                return PolicyEngine().evaluate(
                    endpoint, 'tool:send', 'user', {
                        'tool_args': {
                            'recipient': 'intended'}, 'approval_token': token, }).allowed
            finally:
                connections.close_all()
