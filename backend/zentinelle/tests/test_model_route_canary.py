...
class ModelRouteCanaryTests(TestCase):
    def test_canary_uses_runtime_guard_and_has_no_provider_side_effect(self):
        ...
        with patch('zentinelle.api.views.model_route_canary.get_request_tenant_id', return_value='tenant-a'), \
            ...
            response = client.post('/api/zentinelle/v1/models/route-canary',
                                   {'provider': 'openai', 'model': 'gpt-4o'}, format='json')
        ...
        self.assertEqual(
            ModelRouteCanary.objects.filter(
    ...
    def test_canary_rejects_unbounded_baseline_before_guard(self):
        ...
        with patch('zentinelle.api.views.model_route_canary.get_request_tenant_id', return_value='tenant-a'), \
            ...
            response = client.post(
                '/api/zentinelle/v1/models/route-canary',
                {
                    'provider': 'openai',
                    'model': 'gpt-4o',
                    'baseline': {
    ...
    def test_rollback_is_tenant_scoped_and_idempotency_is_explicit(self):
        ...
        with patch('zentinelle.api.views.model_route_canary.get_request_tenant_id', return_value='tenant-a'), \
            ...
            created = client.post('/api/zentinelle/v1/models/route-canary',
                                  {'provider': 'openai', 'model': 'gpt-4o'}, format='json')
        canary_id = created.json()['id']
        with patch('zentinelle.api.views.model_route_canary.get_request_tenant_id', return_value='tenant-b'):
            self.assertEqual(
                client.post(
                    f'/api/zentinelle/v1/models/route-canary/{canary_id}/rollback').status_code,
                404)
        with patch('zentinelle.api.views.model_route_canary.get_request_tenant_id', return_value='tenant-a'):
            rolled = client.post(
                f'/api/zentinelle/v1/models/route-canary/{canary_id}/rollback')
